...
| Rules (3 files) | ~750 |
| settings.json | ~1,315 |
| **Template subtotal** | **~2,575** |
| Claude Code system prompt | ~3,000-5,000 |
| **Total per turn** | **~5,500-7,500** |

Template-only overhead is ~2,575 tokens (~1.3% of context). With Claude Code's system prompt: ~5,500-7,500 (~3-4%). Negligible either way.

**On-demand (loaded when invoked):**

| Component | ~Tokens |
|-----------|---------|
| Skills (34 total) | ~25,500 |
| Reference docs (9) | ~10,650 |
| Subagents (6) | ~5,550 |

Skills load one at a time, not all at once. A single skill adds ~290-1,150 tokens.
...
- Template overhead: ~2,575 tokens (~1.3% of context), or ~5,500-7,500 with system prompt. Not a concern.
